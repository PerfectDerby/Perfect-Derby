function w(){return typeof window<"u"&&typeof window.Capacitor<"u"&&window.Capacitor.isNativePlatform?.()===!0}const p=typeof window<"u"&&window.Capacitor?.isNativePlatform?.()&&window.Capacitor?.Plugins?.AdMob?window.Capacitor.Plugins.AdMob:null;function c(e,...o){if(p&&typeof p[e]=="function")return p[e](...o);const t={initialize:async()=>{},requestConsentInfo:async()=>({isConsentFormAvailable:!1,status:"NOT_REQUIRED"}),showConsentForm:async()=>{},prepareRewardVideoAd:async()=>{},showRewardVideoAd:async()=>({type:"Dismissed"}),addListener:()=>({remove:()=>{}})};return t[e]?t[e]():Promise.resolve()}const m={initialize:e=>c("initialize",e),requestConsentInfo:e=>c("requestConsentInfo",e),showConsentForm:()=>c("showConsentForm"),prepareRewardVideoAd:e=>c("prepareRewardVideoAd",e),showRewardVideoAd:()=>c("showRewardVideoAd"),addListener:(...e)=>c("addListener",...e)},y="ca-app-pub-3940256099942544/5224354917",v="ca-app-pub-3940256099942544/1712485313";let l=!1,r=null;const A={async init(){if(!l){if(!w()){console.log("[RewardService] Web platform detected. AdMob SDK not loaded (using mock mode)."),l=!0;return}try{r=m,await r.initialize({requestTrackingAuthorization:!0});try{const e=await r.requestConsentInfo({});e.isConsentFormAvailable&&e.status==="REQUIRED"&&(await r.showConsentForm(),console.log("[RewardService] UMP consent form shown."))}catch(e){console.warn("[RewardService] UMP consent error (non-fatal):",e)}l=!0,console.log("[RewardService] AdMob initialized successfully.")}catch(e){console.error("[RewardService] Failed to initialize AdMob:",e),l=!0}}},async showRewardedAd(){if(l||await this.init(),!w())return x();try{const o=window.Capacitor?.getPlatform?.()==="ios"?v:y;return await r.prepareRewardVideoAd({adId:o,isTesting:!0}),new Promise(async t=>{let a=!1;const d=await r.addListener("onRewardedVideoAdReward",i=>{console.log("[RewardService] Ad Reward Received:",i),a=!0}),n=await r.addListener("onRewardedVideoAdDismissed",()=>{console.log("[RewardService] Ad Dismissed. rewarded:",a),d.remove(),n.remove(),t(a)}),s=await r.addListener("onRewardedVideoAdFailedToLoad",i=>{console.warn("[RewardService] Ad Failed to Load/Show:",i),d.remove(),n.remove(),s.remove(),t(!1)});await r.showRewardVideoAd().catch(i=>{console.error("[RewardService] Show failed:",i),d.remove(),n.remove(),s.remove(),t(!1)})})}catch(e){return console.warn("[RewardService] Rewarded ad exception:",e),!1}}};function x(){return new Promise(e=>{const o=document.createElement("div");o.id="mock-ad-overlay",o.style.cssText=`
            position: fixed;
            inset: 0;
